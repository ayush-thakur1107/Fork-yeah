...
export default function Whiteboard({ roomId, username, onLeave }: WhiteboardProps) {
  ...
  const backendUrl = "https://fork-yeah-backend.onrender.com";
  ...
  useEffect(() => {
    ...
    newSocket.on("receive-update", (updatedElement: any) => {
      setElements(prev => {
        const exists = prev.some(e => e.id === updatedElement.id);
        return exists
          ? prev.map(e => e.id === updatedElement.id ? updatedElement : e)
          : [...prev, updatedElement];
      });
    });

    newSocket.on("cursor-move", ({ socketId, x, y }: any) => {
      setCursors((prev: any) => ({
        ...prev,
        [socketId]: { x, y }
      }));
    ...
    <div className={`w-full h-full flex flex-col font-sans relative overflow-hidden transition-colors ${theme === 'dark' ? 'bg-slate-900 text-slate-100' : 'bg-white text-slate-800'}`}>
      ...
      <div className="flex flex-1 overflow-hidden relative">
          ...
          <div className="flex flex-col gap-1 w-full px-2 relative">
            ...
            {tools.map((t) => (
              <button
                ...
                className={`flex flex-col items-center justify-center p-2 rounded-xl transition-colors ${activeTool === t.id ? 'bg-[#e0e7ff] text-[#4338ca] shadow-sm' : 'text-slate-500 hover:bg-slate-50'
                  }`}
              >
        ...
        <main className={`flex-1 relative ${theme === 'dark' ? 'bg-[#0f172a]' : 'bg-[#fafafa]'}`} ref={containerRef} onWheel={handleWheel}>
          {Object.entries(cursors).map(([id, pos]: any) => (
            <div
              key={id}
              style={{
                position: "absolute",
